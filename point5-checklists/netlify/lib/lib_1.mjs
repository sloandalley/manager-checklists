...
// ---------- Email ----------
// Gmail / Google Workspace (GMAIL_USER + GMAIL_APP_PASSWORD) is the default. SendGrid is used only if
// GMAIL_USER is not set and SENDGRID_API_KEY is.
export async function sendEmail({ to, subject, html, text }) {
  if (!to) return { skipped: "no recipient" };
  text = text || html.replace(/<style[\s\S]*?<\/style>/g, "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
  const from = { email: env("FROM_EMAIL", env("GMAIL_USER")), name: env("FROM_NAME", "Point 5 Fun Checklists") };
  ...
  if (env("GMAIL_USER")) {
    ...
    const info = await transport.sendMail({
      from: `"${from.name}" <${from.email}>`, to, subject, text, html,
      replyTo: env("REPLY_TO_EMAIL") || undefined,
    });
    return { sent: to, id: info.messageId };
  }
